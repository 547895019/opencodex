...
async function sidecarVisionResponseSettings(config: OcxConfig): Promise<{
  ...
  const model = backend === "routed"
    ? vs.model ?? "routed"
    : resolveEffectiveVisionModel(config, backend);
  ...
  // remains stricter and rejects a model that is positively proven blind. Routed
  // backend models are arbitrary provider/model ids, so they never join the
  // openai/anthropic catalog lists.
  if (backend !== "routed" && !models.some(option => option.value === model)) {
  ...
  if (url.pathname === "/api/sidecar-settings" && req.method === "PUT") {
    ...
    if (body.vision && typeof body.vision.model === "string" && body.vision.model !== "") {
      ...
      const hint = body.vision.backend === "anthropic" || body.vision.backend === "openai"
        ? body.vision.backend
        : config.visionSidecar?.backend === "routed"
          ? "openai"
    ...
    if (body.vision) {
      ...
      else if (body.vision.backend === "openai" || body.vision.backend === "anthropic" || body.vision.backend === "routed") {
